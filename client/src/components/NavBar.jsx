    ...
    <Navbar variant='light' expand="lg" className="bg-light">
      <Container fluid>
        ...
        <Navbar.Collapse id="responsive-navbar-nav">
          <Nav className="d-flex justify-content-center w-100" style={{marginLeft: '15%'}}>
            ...
            <NavDropdown title="Еще" id="collapsible-nav-dropdown">
              ...
              <NavDropdown.Divider />
              <NavDropdown.Item href="/form">Ближайшие турниры</NavDropdown.Item>
              <NavDropdown.Divider />
          ...
          {isAuthenticated ? (
            <>
              <AdminCheck>
                <Button variant='warning' className='me-2' href='/admin'>Админ</Button>
              </AdminCheck>
              {user && (
                <Nav.Link className="d-none d-sm-inline text-decoration-none text-dark me-2" href="/lk" style={{width: '30%'}}>
                  {user.surname} {user.name}
                  <img className='ms-3' src={`http://localhost:3000/uploads/${user.img}`} alt="user" style={{width: '30px', height: '30px', borderRadius: '50%'}}/>
                </Nav.Link>
              )}
            </>
          ) : (
